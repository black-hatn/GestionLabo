  ...
  if (loading) {
    return (
      <div className="py-20 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm font-semibold text-neutral-500">Chargement de la fiche patient...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 select-none">
      ...
      <div className="flex items-center justify-between border-b border-neutral-200 pb-4">
        ...
        <span className="text-xs font-bold text-neutral-400">N° DOSSIER : {patient.record_number}</span>
      </div>
        ...
        <div className="flex flex-col gap-6">
          <Card className="text-center relative">
            <CardContent className="p-0 flex flex-col items-center">
              ...
              <div className="w-full mt-6 pt-6 border-t border-neutral-100 flex flex-col gap-3.5 text-left text-sm text-neutral-700">
                ...
                {patient.insurance_number && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-xs bg-neutral-100 border px-2 py-0.5 rounded">INS : {patient.insurance_number}</span>
                  </div>
                )}
              </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select
                        ...
                        className="w-full text-sm rounded-lg border border-neutral-300 py-2.5 px-3 focus:ring-2 focus:ring-primary-100 focus:outline-none"
                      >
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between border-none pb-0">
                <div>
                  ...
                  <p className="text-xs text-neutral-500 mt-0.5">Timeline des analyses médicales demandées</p>
                </div>
              </CardHeader>
              <CardContent className="mt-6">
                {demandes.length === 0 ? (
                  <div className="py-12 text-center text-sm font-semibold text-neutral-400 flex flex-col items-center gap-3">
                    <Clock className="w-10 h-10 text-neutral-300 animate-pulse" />
                    Aucune analyse demandée pour ce patient.
                  </div>
                ) : (
                  <div className="relative border-l border-neutral-200 ml-4 flex flex-col gap-8">
                    {demandes.map((d) => (
                      <div key={d.id} className="relative pl-6">
                        {/* Timeline Bullet */}
                        <span className="absolute -left-[11px] top-1.5 flex h-5.5 w-5.5 items-center justify-center rounded-full bg-white border-2 border-primary-500 text-primary-500">
                          <Activity className="w-3 h-3" />
                        </span>
                        
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 bg-neutral-50 border border-neutral-200/50 rounded-xl p-4 hover:shadow-sm transition-all">
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-mono font-bold text-neutral-500">{d.record_number}</span>
                              <span className="text-xs font-semibold text-neutral-400">•</span>
                              <span className="text-xs font-semibold text-neutral-400">{formatDate(d.created_at)}</span>
                            </div>
